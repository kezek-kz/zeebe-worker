package kezek.zeebe.worker.core.api.http.route
...
import kezek.zeebe.worker.core.codec.MainCodec
import kezek.zeebe.worker.core.domain.dto.{DeployedWorkflowsDTO, StartWorkflowInstanceDTO, UploadWorkflowsDTO}
import kezek.zeebe.worker.core.service.ZeebeService
import kezek.zeebe.worker.core.util.HttpUtil
...
trait ZeebeHttpRoutes extends MainCodec {
  ...
  @Operation(
    ...
    requestBody = new RequestBody(
      content = Array(
        new Content(
          schema = new Schema(implementation = classOf[UploadWorkflowsDTO]),
      ...
      path("deploy-workflows") {
        fileUpload("workflow") {
          case (fileInfo, byteSource) => {
            onComplete(zeebeService.deployWorkflow(byteSource, fileInfo)) {
              case Success(result) => complete(result)
              case Failure(exception) => HttpUtil.completeThrowable(exception)
            }
          }
