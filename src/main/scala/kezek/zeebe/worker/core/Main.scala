package kezek.zeebe.worker.core
...
import kezek.zeebe.worker.core.api.http.HttpServer
import kezek.zeebe.worker.core.service.ZeebeService
import kezek.zeebe.worker.core.service.worker.{HttpJobWorker, InitJobWorker, SetVariablesJobWorker}
...
object Main extends App {
  ...
  zeebeClient.newWorker().jobType("http-request").handler(new HttpJobWorker()).open()
  zeebeClient.newWorker().jobType("init").handler(new InitJobWorker()).open()
  zeebeClient.newWorker().jobType("set-variables").handler(new SetVariablesJobWorker()).open()

  implicit val zeebeService: ZeebeService = new ZeebeService()
